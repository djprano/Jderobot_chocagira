...
void Api::RunNavigationAlgorithm() {
	...
	printf("%d -- %d -- %d  Theeta: %i  Estado: %i \n",
			laser->distanceData[174], laser->distanceData[90],
			laser->distanceData[5], (int) encoders->robottheta, estado);
	...
	case 0:        //el robot anda hacia delante
		...
		if (laser->distanceData[90] < 1500 | laser->distanceData[5] < 500 | laser->distanceData[174] < 500) {
			this->setMotorV(0);
			this->setMotorW(0);
	...
	case 2:        //detectamos pared
		...
		if (encoders->robottheta > gradosallegar - 2 && encoders->robottheta < gradosallegar + 2) {
			estado = 0;
			break;
